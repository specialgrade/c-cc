#include <stdio.h>
void insertionAscend(int arr[], int size) {
	
	printf("\nThe changes happened in each iteration are:\n"); // program will show the process of sorting_insertion, elements are compared depending on the condition they've chosen
...
void insertionDescend(int arr[],int size) {
...
int main() {
	...
	if(results==1) { // for calling out the function
		insertionAscend(arr,size); // function call is ascending
	} else if(results==2) {
		insertionDescend(arr,size); // function call is descending
	} else {
